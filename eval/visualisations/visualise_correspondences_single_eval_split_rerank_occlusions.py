...
from dataset.augmentation import Normalize, RandomOcclusion
...
def get_eval_pairs_dataloader(
    ...
    # Append occlusion angle to embeddings to prevent regular embeddings being overwritting
    model_name_embeddings = model_name + f'_OCCLUSION-ANGLE{params.random_occlusion_angle:.0f}'
    if args.load_embeddings:
        database_embeddings, database_local_dict = load_embeddings_from_file(model_name_embeddings, params.dataset_name, location_name, f'database_{args.database_split_idx}')
    else:
    ...
    if args.save_embeddings:
        save_embeddings_to_file(database_embeddings, database_local_dict, model_name_embeddings, params.dataset_name, location_name, f'database_{args.database_split_idx}')

    if args.load_embeddings:
        query_embeddings, query_local_dict = load_embeddings_from_file(model_name_embeddings, params.dataset_name, location_name, f'query_{args.query_split_idx}')
    else:
        query_embeddings, query_local_dict = get_latent_vectors(model, query_set, device, params, only_global=False, show_progress=True)
    if args.save_embeddings:
        save_embeddings_to_file(query_embeddings, query_local_dict, model_name_embeddings, params.dataset_name, location_name, f'query_{args.query_split_idx}')
    ...
    if reranking and params.model_params.rerank_mode is not None:
        if 'egonn' in params.model_params.model.lower():  # Don't need full dataloader for EgoNN
            ...
            rerank_dataloader = make_eval_dataloader_reranking(  # Increased num workers to minimise bottleneck (assumes enough threads are available)
                params, query_set, database_set, global_result_dict['query_nn_list'], num_workers=(args.num_workers * 3)
            )
    ...
    eval_dataloader = make_eval_dataloader_6DOF(
        params, query_set, database_set, metric_loc_pairs_list[eval_mode], num_workers=args.num_workers,
    )
...
def setup_model():
    ...
    # Set random occlusion angle
    params.random_occlusion_angle = args.random_occlusion_angle
...
def main():
    ...
    for ii, local_batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        save_dir_ii = None
        if args.save_dir is not None:
            save_dir_ii = os.path.join(args.save_dir, f'{args.random_occlusion_angle:.0f}deg-occl', f'{query_idx}')
            rerank_mode_str = params.model_params.rerank_mode
            if params.model_params.rerank_mode == 'local_hierarchical_gc':
                rerank_mode_str = 'msgv' 
            save_dir_ii += f'-reranked-{rerank_mode_str}' if reranking else '-initial'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--random_occlusion_angle', type=float, help='Angle of random occlusions to apply')
    ...
    if not (0 < args.random_occlusion_angle < 360):
        raise ValueError
    print(f'Random occlusion angle: {args.random_occlusion_angle}')
