...
Saves only coarse correspondences and registration attempts in a format suitable
for loading via ROS node for hit/miss visualisation.
"""
import argparse
import pickle
import os
...
def main():
    ...
    for ii, local_batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        filename_base = f'{query_idx}' + (f'-reranked' if reranking else '-initial')

        # Save estimated and GT TF
        np.savetxt(
            fname=os.path.join(args.save_dir, f'{filename_base}-est_TF.txt'),
            X=T_estimated.numpy(),
        )
        np.savetxt(
            fname=os.path.join(args.save_dir, f'{filename_base}-gt_TF.txt'),
            X=T_gt.numpy(),
        )
        ...
        if not args.registration_only:
            T_gt_rot = np.eye(4)
            T_gt_rot[:3,:3] = T_gt.numpy()[:3,:3]  # rotation
            T_gt_rot[2,3] = T_gt.numpy()[2,3]  # t_z
            visualise_coarse_correspondences(
                ...
                filename_base=f'{filename_base}-coarse_corr',
        ...
        visualise_registration(
            ...
            window_width=1000,
            save_dir=args.save_dir,
            filename=f'{filename_base}-registration',
            disable_animation=args.disable_animation,
            non_interactive=args.non_interactive,
            voxel_size=args.voxel_size,
        )
