"""
Visualises relay token correspondences from HOTFormerMetricLoc on a single evaluation split.
"""
import argparse
import pickle
import os
...
def main():
    ...
    for ii, local_batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        anc_shift_and_scale = local_batch['anc_shift_and_scale'][0]
        pos_batch = local_batch['pos_batch']
        pos_points = pos_batch['points'].points
        pos_shift_and_scale = local_batch['pos_shift_and_scale'][0]
        T_gt = local_batch['transform'][0]
        with torch.inference_mode():
            anc_output_dict = model(anc_batch, global_only=True)
            pos_output_dict = model(pos_batch, global_only=True)
        anc_octree = anc_output_dict['octree']
        anc_rt_dict = anc_output_dict['rt']
        pos_octree = pos_output_dict['octree']
        pos_rt_dict = pos_output_dict['rt']

        # Unnormalize points
        anc_points = Normalize.unnormalize(anc_points, anc_shift_and_scale)
        pos_points = Normalize.unnormalize(pos_points, pos_shift_and_scale)
        
        # Get relay token centroids
        anc_rt_centroids_dict = {}
        pos_rt_centroids_dict = {}
        for depth_j in anc_rt_dict.keys():
            anc_rt_centroids_dict[depth_j] = anc_octree.window_stats[depth_j][..., :3]
            anc_rt_mask = anc_octree.ct_mask[depth_j][0, 0] == 0.  # Mask of non-padding RTs
            anc_rt_centroids_dict[depth_j] = anc_rt_centroids_dict[depth_j][anc_rt_mask]
            anc_rt_dict[depth_j] = anc_rt_dict[depth_j][anc_rt_mask]
            pos_rt_centroids_dict[depth_j] = pos_octree.window_stats[depth_j][..., :3]
            pos_rt_mask = pos_octree.ct_mask[depth_j][0, 0] == 0.  # Mask of non-padding RTs
            pos_rt_centroids_dict[depth_j] = pos_rt_centroids_dict[depth_j][pos_rt_mask]
            pos_rt_dict[depth_j] = pos_rt_dict[depth_j][pos_rt_mask]
            # Unnormalize RT centroids
            if anc_shift_and_scale is not None:
                anc_rt_centroids_dict[depth_j] = Normalize.unnormalize(
                    anc_rt_centroids_dict[depth_j], anc_shift_and_scale,
                )
            if pos_shift_and_scale is not None:
                pos_rt_centroids_dict[depth_j] = Normalize.unnormalize(
                    pos_rt_centroids_dict[depth_j], pos_shift_and_scale,
                )
