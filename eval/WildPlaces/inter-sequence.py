...
def evaluate(model, device, params, queries, databases, query_features, database_features, args):
    ...
    for query_sets, database_sets, query_feat, database_feat, location in zip(queries, databases, query_features, database_features, args.location_names):
        ...
        if query_feat == None or database_feat == None:
            query_feat = []
            database_feat = []
            for idx in tqdm(range(len(query_sets)), desc='Computing embeddings'):
                query_feat.append(get_latent_vectors(model, query_sets[idx], device, params))
                database_feat.append(get_latent_vectors(model, database_sets[idx], device, params))
        else:
...
if __name__ == '__main__':
    ...
    # Format empty list correctly for zip function
    if args.query_features is None or args.database_features is None:
        args.query_features = [None]*len(args.queries)
        args.database_features = [None]*len(args.databases)        

    stats = evaluate(model, device, params, args.queries, args.databases, args.query_features, args.database_features, args)
    print(stats)
