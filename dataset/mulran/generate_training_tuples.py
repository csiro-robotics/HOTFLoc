...
# Adapted from https://github.com/jac99/Egonn/blob/main/datasets/mulran/generate_training_tuples.py
...
from dataset.mulran.mulran_raw import MulranSequences
from dataset.base_datasets import TrainingTuple
from dataset.mulran.utils import relative_pose
...
def generate_training_tuples(ds: MulranSequences, pos_threshold: float = 10, neg_threshold: float = 50):
    ...
    for anchor_ndx in tqdm.tqdm(range(len(ds))):
        ...
        tuples[anchor_ndx] = TrainingTuple(id=anchor_ndx, timestamp=ds.timestamps[anchor_ndx],
                                           rel_scan_filepath=os.path.join(args.sequence, ds.rel_scan_filepath[anchor_ndx]),  # Add sequence name to rel path, so Sejong and DCC queries will work with the same dataset_root
                                           positives=positives, non_negatives=non_negatives,
                                           position=anchor_pos, pose=anchor_pose,
...
if __name__ == '__main__':
    ...
    if args.save_dir is not None:
        ...
        train_tuples_filepath = os.path.join(args.save_dir, pickle_name)
    else:
    ...
    if args.save_dir is not None:
        test_tuples_filepath = os.path.join(args.save_dir, pickle_name)
    else:
