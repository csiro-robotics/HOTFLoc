...
# Adapted from https://github.com/jac99/Egonn/blob/main/datasets/southbay/southbay_raw.py
...
class SouthbayPointCloudLoader(PointCloudLoader):
    def set_properties(self):
        ...
        self.max_range = 80  # was originally 100 in EgoNN, but make 80 for consistency with MulRan (since octrees are normalised)

    def read_pc(self, file_pathname):
        # Returns Nx3 tensor
        pc_o3d = o3d.io.read_point_cloud(file_pathname)
        pc = np.asarray(pc_o3d.points, dtype=np.float64)
        pc = np.float32(pc)
        # Replace nans with all-zero coords
        nan_mask = np.isnan(pc).any(axis=1)
        pc[nan_mask] = np.array([0., 0., 0.], dtype=np.float32)
        return pc
    ...
    def read_poses(self):
        with open(self.pose_filepath) as h:
            ...
            for ndx, row in enumerate(csv_reader):
                ...
                qz = float(row[7])
                qw = float(row[8])
                # Expects quaternions in x, y, z, w format
                se3 = poses.xyz_quat2m(np.array((x,y,z,qx,qy,qz,qw)))
...
class SouthBayDataset:
    ...
    def index_split(self, split):
        ...
        locations.sort()
        for loc in tqdm.tqdm(locations, desc='Processing locations'):
