...
def load_pc_data_set(data_set):
    ...
    for i in range(len(data_set)):
        if len(data_set[i]) == 0:   # handle above_under empty database and query sets
            pc = np.array([])
        else:
    ...
    for i in range(len(queries_output)):
        ...
        for j in range(len(indices[0])):
            if indices[0][j] in true_neighbors:
                # if check(i, indices[0][j], n, m) is False: # causes errors, also not used for eval during training
                #     continue
                if j == 0:
