...
def load_pc_file(filename, dataset_folder, input_dim=3, num_points=4096):
    ...
    pc_path = os.path.join(dataset_folder, filename)
    pcd_flag = False
    if os.path.splitext(pc_path)[-1] == ".pcd": # compatibility for above-under .pcd      
        pcd_flag = True
        pc_o3d = o3d.io.read_point_cloud(pc_path)
        pc = np.asarray(pc_o3d.points, dtype=np.float64)        
    else:
        pc = np.fromfile(pc_path, dtype=np.float64)
    
    if input_dim == 3:
        if pcd_flag and pc.shape == (num_points, input_dim): # above-under pcd already in Nx3 format
            return pc
