...
class NetVLADBase(nn.Module):
    ...
    def forward(self, x):
        max_samples = x.shape[2]
        x = x.transpose(1, 3).contiguous()                          # B x 1024 x N x 1 -> B x 1 x N x 1024
        x = x.view((-1, max_samples, self.feature_size))       # B x N x 1024
        ...
        if self.add_batch_norm:
            ...
            activation = activation.view(-1, max_samples, self.cluster_size)   # BN x 64 -> B x N x 64
            # activation = activation.transpose(1,2).contiguous()
        else:
        ...
        activation = activation.view((-1, max_samples, self.cluster_size))     # B x N x 64
        ...
        x = x.view((-1, max_samples, self.feature_size))   # B x N x C -> B x N x C
...
class SpatialPyramidNetVLAD(nn.Module):
    def __init__(self, feature_size, cluster_size, output_dim,
        ...
        # max_samples[0] = 64
        self.vlad0 = NetVLADBase(feature_size[0], cluster_size[0], output_dim[0], gating, add_batch_norm)
        # max_samples[1] = 256
        self.vlad1 = NetVLADBase(feature_size[1], cluster_size[1], output_dim[1], gating, add_batch_norm)
        # max_samples[2] = 1024
        self.vlad2 = NetVLADBase(feature_size[2], cluster_size[2], output_dim[2], gating, add_batch_norm)
        # max_samples[3] = 4096
        self.vlad3 = NetVLADBase(feature_size[3], cluster_size[3], output_dim[3], gating, add_batch_norm)
