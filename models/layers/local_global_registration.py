...
(https://github.com/qinzheng93/GeoTransformer/blob/main/geotransformer/modules/geotransformer/local_global_registration.py)

Added logging of various registration stats.
...
class LocalGlobalRegistration(nn.Module):
    ...
    def compute_correspondence_matrix(self, score_mat, ref_knn_masks, src_knn_masks):
        ...
        if self.use_dustbin:
            # NOTE: This seems wildly incorrect, this indexes only the dustbin column, rather than removing it
    ...
    def forward(
        ...
        Returns:
            ref_corr_points: torch.Tensor (C, 3)
            src_corr_points: torch.Tensor (C, 3)
            corr_scores: torch.Tensor (C,)
            estimated_transform: torch.Tensor (4, 4)
            num_corr_points: List (B',)
            best_ref_corr_points: torch.Tensor (C', 3)
            best_src_corr_points: torch.Tensor (C', 3)
            best_transform: torch.Tensor (4, 4)
        """
