...
def get_parser():
    ...
    if cfg["DATA_TYPE"] == "baseline":
        ...
        test_baseline = "test_queries_baseline_v1.pickle"
    elif cfg["DATA_TYPE"] == "campus3d":
        train_baseline = "training_queries_umd_4096.pickle"
        test_baseline = "umd_evaluation_query_PNVLAD_format.pickle"
    elif cfg["DATA_TYPE"] == "above-under_baseline":
        train_baseline = "training_queries_above-under_baseline_v1.pickle"
        test_baseline = "test_queries_above-under_v1.pickle"
    elif cfg["DATA_TYPE"] == "above-under_refined":
        train_baseline = "training_queries_above-under_refined_v1.pickle"
        test_baseline = "test_queries_above-under_v1.pickle"
    else:
        logger.info("DATA_TYPE is not support, only support: 'baseline', 'refine', 'campus3d', 'above-under', and 'lpd_baseline'")
        exit()
...
def init():
    # global variables
    global args, logger, event_writer
    ...
    args = get_parser()
    logger = get_logger()
    event_writer = SummaryWriter(os.path.join(args["save_path"], 'events'))
...
def train_one_epoch(model, optimizer, train_writer, loss_function, epoch):
    ...
    # Shuffle train files, ignore faulty tuples in batches
    train_file_idxs = []
    for idx, query_tuple in TRAINING_QUERIES.items():
        if len(query_tuple["positives"]) >= args["TRAIN_POSITIVES_PER_QUERY"]:   # keep only valid queries
            train_file_idxs.append(idx)
    train_file_idxs = np.array(train_file_idxs)
    ...
    for i in range(iter_num):
        ...
        # log to tensorboard
        event_writer.add_scalar("Loss", loss_meter.val, TOTAL_ITERATIONS)
...
def eval(model, epoch):
    ...
    for m in range(len(QUERY_SETS)):
        for n in range(len(QUERY_SETS)):
            if (m == n and args["SKIP_SAME_RUN"]) or len(DATABASE_VECTORS[m]) == 0 or len(QUERY_VECTORS[n]) == 0: 
                continue
    ...
    # log to tensorboard
    event_writer.add_scalar("Average Recall 1", ave_recall[0], epoch)
    event_writer.add_scalar("Average Recall 5", ave_recall[4], epoch)
    event_writer.add_scalar("Average Recall 1percent", ave_one_percent_recall, epoch)
    event_writer.add_scalar("Average Similarity", average_similarity, epoch)
...
def get_latent_vectors_for_test(model, dict_to_process, data):
    ...
    for q_index in range(len(train_file_idxs)//batch_num):
        ...
        q_output.append(out)
    logger.info("inference time: %f" % (times/(cnt + 1e-10)))
